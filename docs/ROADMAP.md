...
- [ ] Transaction builder: encode the ZK-program `Verify*` + Token-2022
      ...
      sequence the multi-tx flow — so the SDK can land a transfer end-to-end. The
      proofs are now generated by the SDK; this layer wires them into
      transactions (today submission is driven by the spl-token CLI on the fork).
